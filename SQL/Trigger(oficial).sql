...
FOR EACH ROW BEGIN
    ...
    VALUES (CONCAT(NEW.fk_Empresa_CNPJ, ' - ', NEW.fk_Manutencao_numeroregistro), 'INSERIDO');
END//

CREATE TRIGGER trg_requisita_update AFTER UPDATE ON requisita
FOR EACH ROW BEGIN
    INSERT INTO log_requisita (chave_primaria, acao)
    VALUES (CONCAT(NEW.fk_Empresa_CNPJ, ' - ', NEW.fk_Manutencao_numeroregistro), 'ALTERADO');
END//

CREATE TRIGGER trg_requisita_delete AFTER DELETE ON requisita
FOR EACH ROW BEGIN
    INSERT INTO log_requisita (chave_primaria, acao)
    VALUES (CONCAT(OLD.fk_Empresa_CNPJ, ' - ', OLD.fk_Manutencao_numeroregistro), 'REMOVIDO');
END//
